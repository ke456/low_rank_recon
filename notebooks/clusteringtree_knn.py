...
class ClusteringTree_knn:
    ...
    def cluster_one_variable(self, data, cur_dim):
        ...
        for x in self.linspace:
            ...
            dist_to_c2 = 0
            for d in d1:
                dist_to_c1 += np.linalg.norm(d-c1)/max_dist
            for d in d2:
                dist_to_c2 += np.linalg.norm(d-c1)/max_dist

            # compute knn of points and define a point to be clustered wrt
            # a boundary if the majority of its neighbours are within the
            # same cluster
            # left partition
            for i in range(len(d1)):
                ...
                for j in range(len(d1)):
                    dd = d1[j]
                    if i != j:
                        dist.append((np.linalg.norm(dd-d)/max_dist,1))
                for j in range(len(d2)):
                    dd = d2[j]
                    dist.append((np.linalg.norm(dd-d)/max_dist, 0))
                dist.sort()
                count = 0
                for t in range(min(len(dist),k)):
                    if dist[t][1] == 1:
                        count += 1
                if count >= np.floor(k/2):
                    n_clustered += 1
                else:
                    print("dist:", dist[:5])

...
void run_test(Table &training, Table &test, Tuple &costs, string to_runs){ 
  ...
  cout << "starting pre-trained RL test" << endl;
  for (int bc = 0; bc < N; bc++){
    for (int i = 0; i < test.size(); i++){
      ...
      dist_trained[bc] += knn_total_distance(training, t, test[i], 5);
    }
  }
  ...
  cout << "starting pre-trained tree test" << endl;
  for (int bc = 0; bc < N; bc++){
    for (int i = 0; i < test.size(); i++){
      // set everything to be unknown
      auto t = test[i];
      ...
      double budget = (bc+1.0)/N;
      string ap = add( add("_tree", to_string(bc+1))  , ".csv");
      string fname = add(to_runs,ap);
      trained_agent ta(fname);
      ta.set_costs(&costs);
      ta.set_budget(budget);
      ...
      ta.set_index(i);
      while(ta.next_action()){} 
      dist[bc] += knn_total_distance(training, t, test[i], 5);
    }
  } 
  ...
  for (int bc = 0; bc < N; bc++){
    for (int i = 0; i < test.size(); i++){
      ...
      ta.set(&t, &test[i]);
      while(ta.next_action()){} 
      dist_rand[bc] += knn_total_distance(training, t, test[i], 5);
    }
  }
